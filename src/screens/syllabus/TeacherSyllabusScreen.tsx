...
// ─── Main Screen (read-only view; CRUD lives in ManageSyllabusScreen) ──────────
const TeacherSyllabusScreen = ({ navigation }: any) => {
  ...
  const loadCombos = useCallback(async () => {
    ...
    try {
      ...
      setCombos(list);
      if (list.length > 0) {
        // Keep the current selection across focus reloads if still present.
        setSelected(prev => {
  ...
  const openManage = () =>
    navigation.navigate('ManageSyllabus', { comboKey: selected?.key });
              ...
              {chaptersLoading ? (
                ...
                <View style={s.empty}>
                  ...
                  <Text style={s.emptySubText}>
                    Tap the edit icon above to add chapters and topics.
                  </Text>
                </View>
              ) : (
